...
##' @param num A value of number of consecutive data points in the moving window used to calculate the slope of the windows and flat areas for extracting Isc and Rsh. The default is 75.
...
IVfeature = function(I, V, crt = 0.2, num = 75, crtvalb = 0.3){
  ...
  for (j in 1:(length(diffslprate) - 1)){
    if (is.nan(diffslprate[j]) | is.nan(diffslprate[j + 1]) | abs(diffslprate[j]) < crt & abs(diffslprate[j + 1]) < crt){
      record2[m,1] = j
      m = m + 1
    }else{
      m = m
